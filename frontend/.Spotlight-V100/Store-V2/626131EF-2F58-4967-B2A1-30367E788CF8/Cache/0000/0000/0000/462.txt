...
// AppSidebar.tsx: Sidebar für OTSM mit geschützter /profile-Route.
...
const AppSidebar: React.FC = () => {
  ...
  const renderMenuItems = (
    ...
    <ul className="flex flex-col gap-1">
      {navItems
        .filter((nav) => nav.path !== "/profile" || (status === "authenticated" && session)) // Verstecke /profile, wenn nicht eingeloggt
        .map((nav, index) => (
          <li key={nav.name}>
            {nav.subItems ? (
              <button
